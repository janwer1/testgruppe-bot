...
The application uses a Formal DI pattern to manage dependencies and configuration:
...
Environment variables are validated via `zod` in `src/env.ts` and mapped to a `BotConfig` object. This ensures all components have access to validated configuration without global state side-effects.
